...
# 将代码变更的文本特征（词袋模型）与软件变更指标（如代码行数、复杂度等）融合
def get_combined_df(code_commit, commit_id, label, metrics_df, count_vect, style='concat'):
    code_df = pd.DataFrame()
    code_df['commit_id'] = commit_id   # 提交ID列
    code_df['code'] = code_commit      # 代码变更文本列
    code_df['label'] = label           # 标签列

    code_df = code_df.sort_values(by='commit_id') # 按提交ID排序

    metrics_df = metrics_df.sort_values(by='commit_hash')   # 按提交ID排序（假设与commit_id对应）
    metrics_df = metrics_df.drop('commit_hash', axis=1)     # 删除重复的提交ID列

    code_change_arr = count_vect.transform(code_df['code']).astype(np.int16).toarray()  # count_vect 将代码文本转为稀疏词频矩阵
    metrics_df_arr = metrics_df.to_numpy(dtype=np.float32)   # 转为NumPy数组

    if style == 'manual':
        final_features = metrics_df_arr     # 仅使用指标特征
    elif style == 'semantic':
        final_features = code_change_arr    # 仅使用文本特征N-gram
    else:
        final_features = np.concatenate((code_change_arr, metrics_df_arr), axis=1)  # 词表大小 + 指标数量
...
def run_experiment(cur_proj, style='concat'):
    # 设置数据与模型存储路径（注意：style参数默认值可能是拼写错误，应为'concat'）
    data_path = 'data/jitline/'  # 数据存储根目录
    model_path = 'model/jitline/final_model/'  # 模型存储根目录
    create_path_if_not_exist(data_path) # 确保目录存在（不存在则创建）
    create_path_if_not_exist(model_path)
    create_path_if_not_exist(data_path + style)
    create_path_if_not_exist(model_path + style)
    # 加载训练/验证/测试集的代码文本、提交ID及标签
    ...
    # 加载训练/验证/测试集的软件变更指标（如代码行增删`la`/`ld`等统计特征）
    ...
    # 初始化词袋模型：过滤低频词（min_df=3），仅使用单词语（ngram_range=(1,1)）
    count_vect = CountVectorizer(min_df=3, ngram_range=(1, 1))
    count_vect.fit(train_code) # 仅用训练集文本拟合词袋模型
    print(f'size of vocab {len(count_vect.get_feature_names_out())}') # 输出词表大小

    # 生成组合特征矩阵（代码文本的稀疏词频 + 软件变更指标）
    # `style`参数控制特征组合方式：'manual'仅指标，'semantic'仅文本，'concat'合并
    train_feature, train_commit_id, new_train_label = get_combined_df(train_code, train_commit, train_label,
                                                                      train_commit_m etrics, count_vect, style)
    ...
    final_train_feature = train_feature         # 最终训练集特征
    final_new_train_label = new_train_label     # 最终训练集标签

    valid_feature = valid_feature               # 独立验证集特征
    valid_label = new_valid_label               # 独立验证集标签

    print('load data of', cur_proj, 'finish')

    # 使用差分进化算法优化SMOTE的k_neighbors参数（搜索范围1-20）
    bounds = [(1, 20)]
    print('executing differential_evolution')

    result = differential_evolution(objective_func, bounds,
                                    args=(final_train_feature, final_new_train_label,valid_feature, valid_label),
                                    popsize=5,         # 种群大小
                                    mutation=0.7,       # 变异概率
                                    recombination=0.3,  # 交叉概率
                                    maxiter = 10,       # 最大迭代次数
                                    workers=2,         # 利用所有可用的核心
                                    seed=0)             # 随机种子

    # 应用优化后的k_neighbors参数进行SMOTE过采样（解决类别不平衡）
    print('executing smote')
    smote = SMOTE(random_state=42, k_neighbors=int(np.round(result.x)))  # 使用优化后的k值（四舍五入取整）
    train_feature_res, train_label_res = smote.fit_resample(final_train_feature, final_new_train_label)
    # 初始化随机森林分类器（300棵树，并行计算）
    clf = RandomForestClassifier(n_estimators=300, random_state=42, n_jobs=-1)
    clf_name = 'RF'
    print('building model')
    # 训练模型并在测试集评估，返回训练后的模型和预测结果
    ...
    # 保存预测结果到CSV文件（路径格式示例：data/jitline/concat/changes_RF_DE_SMOTE_min_df_3_prediction_result.csv）
    style += '/'
