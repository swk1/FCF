...
from codes.utils.Metrics import eval_result
...
def run_JITDP():
    ...
    # Get the dataset
    raw_datasets = get_dataset_dict(code_change_args, tokenizer, JITDPDataset)
    is_regression = False
    ...
    model_class, tokenizer_class = cfg.MODEL_CLASSES[model_args.model_type]
    model = model_class(model, config, tokenizer, model_args)
    ...
    def compute_metrics(p: Union[EvalPrediction, PredictionOutput]):
        preds = p.predictions[0] if isinstance(p.predictions, tuple) else p.predictions
        preds = np.squeeze(preds)
        best_threshold = 0.5

        y_preds = preds > best_threshold
        y_preds = list(map(lambda x: int(x), y_preds))
    ...
    if training_args.do_predict:
        ...
        for predict_dataset, task in zip(predict_datasets, tasks):
            ...
            preds = predictions.predictions[0] \
                if isinstance(predictions.predictions, tuple) else predictions.predictions
            preds = np.squeeze(preds)
            best_threshold = 0.5
            ...
            for idx in range(len(eval_dataset)):
                ...
                pred = y_preds[idx]
                prob = preds[idx]
                result.append([example["commit_id"], prob, pred, example["labels"]])

            RF_result = pd.DataFrame(result)
            RF_result.to_csv(os.path.join(training_args.output_dir, "predictions.csv"), sep='\t', index=None)

            eval_result(os.path.join(training_args.output_dir, "predictions.csv"),
                        code_change_args.test_data_file.split("&")[-1])
