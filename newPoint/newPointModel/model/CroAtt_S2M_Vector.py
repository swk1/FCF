...
class RobertaClassificationHead(nn.Module):
    """
    Cross-Attention Fusion Head (S→M, Vector-level)
    - Manual features are compressed into a single vector (B, H)
    - Cross-Attn: Query = CLS (B,1,H), Key/Value = manual vector (B,1,H)
    - Transformer-block style: Residual+LN + FFN+Residual+LN
    ...
    def __init__(self, config):
        ...
        self.manual_ln = nn.LayerNorm(self.feature_size)  # across F
    ...
    def _build_manual_vector(self, manual_features: torch.Tensor) -> torch.Tensor:
        """manual_features: (B, F) -> manual vector: (B, H)"""
    ...
    def forward(self, features, manual_features=None, **kwargs):
        ...
        return logits: (B, 1)
        """
        x0 = features[:, 0, :]  # original CLS (B,H)
        ...
        y_vec = self._build_manual_vector(manual_features)  # (B,H)

        # ===== Transformer Block: Cross-Attn =====
        q = self.q_ln(x0).unsqueeze(1)        # (B,1,H)
        kv = self.kv_ln(y_vec).unsqueeze(1)   # (B,1,H)
