...
class RobertaClassificationHead(nn.Module):
    ...
    def forward(self, features, manual_features=None, **kwargs):
        x = features[:, 0, :]  # 取CLS标记对应的语义特征（代码变更表示）
        y = manual_features.float()  # 手工特征（如代码度量特征）
        y = self.manual_dense(y) # 通过线性层投影到与语义特征相同维度
        y = torch.tanh(y) # 激活函数处理

        # 特征融合：连接操作（concatenation）
        x = torch.cat((x, y), dim=-1) # 在最后一个维度上连接
        x = self.dropout(x)
